...
def tokenize_dataset(
    ds_dict: DatasetDict, tokenizer, max_length: int = 128
) -> DatasetDict:
    """
...
def make_training_args(
    ...
    args.eval_strategy = (
        ...
        else args.eval_strategy
    )
    args.save_strategy = (
        args.save_strategy.value
...
def evaluate_classifier(trainer: Trainer, tokenized_test) -> dict:
    ...
    per_class_f1_arr = f1_score(labels, pred_idx, average=None)
    per_class_precision_arr = precision_score(
        labels, pred_idx, average=None, zero_division=0
    )
    per_class_recall_arr = recall_score(labels, pred_idx, average=None, zero_division=0)

    id2label = trainer.model.config.id2label
    per_class_f1 = {
        id2label[i]: float(per_class_f1_arr[i]) for i in range(len(per_class_f1_arr))
    }
    per_class_precision = {
        id2label[i]: float(per_class_precision_arr[i])
        for i in range(len(per_class_precision_arr))
    }
    per_class_recall = {
        id2label[i]: float(per_class_recall_arr[i])
        for i in range(len(per_class_recall_arr))
    }
...
def main() -> None:
    ...
    trainer = train_classifier(
        tokenized, model_name, training_args, tokenizer, num_labels=3
    )
    ...
    # Predictions CSV — includes per-class prob columns
    ...
    for class_idx, class_name in id2label.items():
        df_out[f"prob_{class_name}"] = [
            float(pred_probs[i, class_idx]) for i in range(len(pred_idx))
        ]
    df_out.to_csv("predictions.csv", index=False)

    # Confusion matrix — printed and saved as CSV
    ...
    if os.environ.get("DATA_PATH") is None:
        ...
        try:
            ...
            print(f"\nPushed to https://huggingface.co/ameralmajali1/{repo_id}")
